...
export const load: PageServerLoad = async ({ cookies }) => {
    ...
    const getApps = async()=>{
        if (type == 'S') {
            return await mysqlconn.query(`SELECT * FROM application WHERE sponsor_id = "${id}" AND app_status = 'A' OR app_status = 'R'`).then(([rows,_])=> rows);
        }
        else if (type == 'A') {
            return await mysqlconn.query(`SELECT * FROM application WHERE app_status = 'A' OR app_status = 'R'`).then(([rows,_])=> rows);
        }
    }
    return {app:getApps()}
};
...
export const actions = {
    ...
    reject: async ({ request }) => {
        ...
        const body1 = await mysqlconn.query(`INSERT INTO audit_log (user_id, action, date, application_sponsor_id, app_id, app_status, app_reason) VALUES ("${data.get('driverID')}", "Application Rejected", NOW(), "${data.get('appID')}", "${data.get('sponsor')}", 'R', "${data.get('rejectreason')}")`);
